...
class EggReport extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final medical = Get.find<EggController>();
    medical.loadEgg(controller.challid.value);
        ...
        children: [
          ...
          Obx(
            () => medical.isloading.isTrue
                ...
                : medical.eggmodellist.isEmpty
                    ? Center(
                        child: Text(nodatafound.tr),
                      )
                    : ListView.builder(
                        physics: NeverScrollableScrollPhysics(),
                        itemCount: medical.eggmodellist.length,
                        shrinkWrap: true,
                        itemBuilder: (context, index) {
                          EggModel m = medical.eggmodellist[index];
                            ...
                            child: Row(
                              children: [
                                ...
                                SingelColumnTable(
                                    fl: 2,
                                    header: false,
                                    text: (m.rate * m.qty).toString()),
                                SingelColumnTable(
                                  fl: 1,
                                  header: false,
                                  text: 'X',
                                  onPressed: () async {
                                    await medical.deleteEgg(m.id);
                                    await medical
                                        .deleteEgg(controller.challid.value);
                                  },
                                ),
                              ],
                            ),
